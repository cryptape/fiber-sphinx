...
pub const ONION_DATA_LEN: usize = 1300;
...
pub fn generate_padding_data(pad_key: &[u8]) -> [u8; ONION_DATA_LEN] {
    use chacha20::{
        cipher::{KeyIvInit, StreamCipher},
        ChaCha20,
    };
    let mut cipher = ChaCha20::new(pad_key.into(), &[0u8; 12].into());
    let mut buffer = [0u8; ONION_DATA_LEN];
...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(packet.into_bytes().to_lower_hex_string(), hex);
    }
